...
We will use the \textit{maximum mean discrepancy} (MMD) as our main metric to measure
 ...
 \begin{boxitemize}
   \begin{itemize}[leftmargin=*, font= \color{blue} \large, label= $\bullet$]
     \item \textbf{For all $f$ in $\H$, we have $p[f] = \bigps{f}{\mu_p}$.}
     \begin{align*}
       p[f] &= \int_{\X} f(x)p(x) \d x\\
       &= \int_{\X} \delta_x[f] p(x) \d x\\
       &= \int_{\X} \bigps{f}{\Phi(x)} p(x) \d x\\%TODO Not obvious
       &=  \bigps{f}{\int_{\X} \Phi(x)p(x) \d x} \\
       &= \bigps{f}{\mu_p}
     \end{align*}
...
\label{sec:BQ}
Let's place a functional prior on the integrand $f$. We will assume that $f$ to be
