...
density with respect to the Lebesgue measure on $\X$ and $f : \X \rightarrow \R$
 ...
 $\acc{w_i}$ :
  ...
  \item [Quasi-Monte Carlo]:$w_i = \frac{1}{n}$ and $x_i$ are chosen according to a low-discrepancy sequence. The Quasi-Monte Carlo method converges with rate $\O (\sqrt { N } )$ whereas the classical Monte Carlo method converges with rate $\O(N)$.
 \item [Kernel herding]: the sequences $(w_i)$ and $(x_i)$ are updated according to the following equations : $x_{t+1} = \underset { x \in \chi}{ argmax } \left< { w }_{ t },\phi (x) \right> $ and $w_{t+1} = { w }_{ t }+{ \Expec{} }_{ x\sim p }\left[ \phi (x) \right] -\phi ({ x }_{ t+1 })$. This method has the same rate of convergence than the Quasi-Monte Carlo method.
...
\end{center}
\noindent
We start with a smooth function $f$ that is the sum of 3 two-dimensional gaussians. Let us notice that the function $f$ intervenes at the end of our algorithms. In other words, we can change $f$ in order to obtain someother values of the integral without having to relaunch everything.
\begin{figure}[H]
\begin{center}
...
\end{figure}
\noindent
The next figures shows the 40 selected points in the FWBQ (FW algorithm) and the FWLSQB (FWLS algorithm) algorithms.
\begin{figure}[H]
\begin{center}
...
\end{figure}
\noindent
Adding the line-search allows for a faster convergence towards the mean element.
