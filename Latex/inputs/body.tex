...
domain $\G$ which is supposed to be a compact and convex space of functions.
...
\begin{mydescription}
  ...
  step-size sequence $\acc{\rho_i}_{i=1}^n$.
  \item [Iteration] For $i=2$ to $n$
  \begin{mydescription}[0.5cm]
    ...
    \begin{boxcomputation}
      \begin{equation}
        \begin{array} { c } { \text { minimize }  \acc{J \left( g _ { k } \right) + \nabla J \left( g _ { k } \right) ^ { \mathrm { T } } \left( g - g _ { k } \right)} } \\ { \text { subject to } g \in \G } \end{array}
      \end{equation}
    \end{boxcomputation}
...
\draw [fill=blue] (-7.294,-2.47) circle (2pt) node[shift={(-1.5ex,-1.5ex)}] {$\mathbf{\overline{g_2}}$};
\draw [fill=blue] (-5.41,-1.47) circle (2pt) node[shift={(0ex,-2.5ex)}] {$\mathbf{g_2}$};
\draw [fill=blue] (-7.10,-0.846) circle (2pt) node[shift={(-1.5ex,1.5ex)}] {$\mathbf{\overline{g_3}}$};
