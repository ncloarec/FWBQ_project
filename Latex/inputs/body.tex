...
\begin{boxtheorem}
  For all $n \geq 2$ :
  \begin{equation}
    g_n = \sum_{k=1}^n \underbrace{ \rho_k \acc{\prod_{k<i\leq n} (1-\rho_i)} }_{:=w_k^{FW}} \overline{g_k} = \sum_{k=1}^n w_k^{FW}\overline{g_k}
  \end{equation}
  with $\rho_1 = 1$.
\end{boxtheorem}
...
\begin{equation*}
  g _ { n } = \sum _ { i = 1 } ^ { n } \rho _ { i - 1 }\acc{ \prod _ { j = i + 1 } ^ { n } \left( 1 - \rho _ { j - 1 } \right) } \overline { g } _ { i }
\end{equation*}
with $\rho_0=1$. 
